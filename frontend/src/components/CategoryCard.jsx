...
function CategoryCard({ cat }) {
  return (
    <Card className="relative w-[120px] h-[120px] md:w-[180px] md:h-[180px] shrink-0 cursor-pointer max-w-sm rounded-2xl overflow-hidden bg-white shadow-xl shadow-gray-200 hover:shodow-lg transition-shadow">
        ...
        src={cat.image} alt="" />
        <div className="absolute bottom-0 w-full left-0 bg-[#ffffff96] bg-opacity-95 px-3 py-1 rounded-t-xl text-center shadow text-sm font-medium text-gray-800 backdrop-blur">{cat.category}</div>
    </Card>
  );
}
